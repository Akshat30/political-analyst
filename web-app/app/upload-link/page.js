      ...
      <div className="w-full h-full py-3 flex flex-col items-center gap-[335px]">
          <div className="w-full max-w-[1328px] h-[507px] flex items-start gap-[22px]">
              ...
              <div className="w-[414px] h-full p-6 bg-zinc-100 rounded-[20px] flex flex-col items-start gap-2.5">
                <div className="text-black text-xl font-bold leading-normal mb-2">Verum</div>
                ...
                <div className="flex flex-col items-start gap-16">
                  <div className="flex flex-col items-start gap-6">
                    {['text', 'link'].map((type, idx) => (
                      <Link href={`/upload-${type}`} className="flex items-center gap-4" key={idx}>
                        ...
                        <div className="text-black text-lg font-bold leading-normal">Upload {type.charAt(0).toUpperCase() + type.slice(1)}</div>
                      </Link>
                    ))}
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-[9px] pt-20">
                    <div className="text-center text-black text-base font-medium">Don’t have a document to upload?</div>
                    <div className="w-full px-9 py-4 rounded-[20px] border-2 border-black flex items-center justify-center ">
                      <div className="justify-center text-black text-lg font-medium leading-normal">View Our Library</div>
                    </div>
                  </div>
                </div>
            </div>
                  ...
                  {['Link Entry', 'Verum'].map((title, idx) => (
                      <div className="w-full flex flex-col items-start" key={idx}>
                          <div className={`h-[50px] pl-[31px] pr-8 py-[13px] bg-violet-700 rounded-t-[20px] flex items-center`}>
                              <div className="text-zinc-100 text-base font-bold leading-normal">{title}</div>
                          </div>
                          <div className={`w-full ${title === 'Link Entry' ? 'h-[223px]' : 'h-36'} p-6 bg-zinc-100 rounded-b-[20px] rounded-r-[20px] flex flex-col items-start gap-6`}>
                              <div className="text-black text-base font-medium leading-normal">{title === 'Link Entry' ? 'Copy/paste or type in bodies of text you wish to analyze.' : 'Upload a text, link, file to get started'}</div>
                              ...
                              {title === 'Link Entry' && (
                                  <div className="w-full flex items-center justify-end gap-2.5">
                                      <div className="p-3 rounded-[20px] border-2 border-violet-700 hover:bg-violet-200 flex items-center gap-4">
                                          <button className="text-black text-lg font-semibold leading-normal"
